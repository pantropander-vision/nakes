...
export async function GET(request: NextRequest) {
  try {
    ...
    const experiences = db.prepare('SELECT * FROM experiences WHERE user_id = ? ORDER BY is_current DESC, start_date DESC').all(user.id);
    const education = db.prepare('SELECT * FROM education WHERE user_id = ? ORDER BY end_year DESC').all(user.id);
    const skills = db.prepare('SELECT * FROM skills WHERE user_id = ? ORDER BY endorsements DESC').all(user.id);
