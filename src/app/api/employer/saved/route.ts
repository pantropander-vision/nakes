...
export async function POST(request: NextRequest) {
  ...
  const db = getDb();
  try {
    await db.prepare('INSERT INTO saved_candidates (employer_id, candidate_id, notes) VALUES (?, ?, ?)').bind(payload.userId, candidate_id, notes || null).run();
...
export async function DELETE(request: NextRequest) {
  ...
  const db = getDb();
  await db.prepare('DELETE FROM saved_candidates WHERE employer_id = ? AND candidate_id = ?').bind(payload.userId, candidate_id).run();
